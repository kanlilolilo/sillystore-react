...
function SidebarLeft() {
  ...
  return (
    <div className="lg:relative max-lg:fixed">
      <div className="absolute fixed">
        ...
        <div
          ...
          lg:-m-6 -m-6 lg:flex flex-col items-center w-56 h-[calc(100vh-94px)] bg-[#ffffff] p-4 px-6 text-center sticky top-[92px] border-solid border-r-8 border-[#3D58BA] overflow-y-auto`}>
        <div className="mb-3 sticky top-0">
          <Button size="small" variant="contained" onClick={handleResetFilters}>
            Reset Filter
          </Button>
        </div>
        <div className="lg:hidden absolute top-0 right-0 cursor-pointer p-4" onClick={toggleSidebar}>
          ✖
        </div>
        <div className="w-full">
          ...
          {Object.keys(colors).map((color) => (
            <label key={color} className="flex items-center block text-left text-sm mb-1">
              ...
              />
              <span className="ml-2">{color.charAt(0).toUpperCase() + color.slice(1)}</span>
            </label>
          ))}
